...
def init_db():
    ...
    # Solo crear admin si la BD está vacía — sin datos demo
    if User.query.count() == 0:
        ...
        db.session.commit()
        print('✅ Admin creado. Ejecuta /admin/reset-passwords para cargar todos los usuarios.')
...
@rol_requerido('administrador')
def reset_passwords():
    """Sincroniza usuarios definitivos — usernames, roles y contraseñas."""
    from werkzeug.security import generate_password_hash
    USUARIOS = [
        ('Administrador',       'admin',                'administrador',    True,  'Admin2026!1'),
        ('Rubí Arizmendi',      'Rubí_Arizmendi',       'aux_comercial',    True,  'AuxCom2026!1'),
    ...
    for nombre, username, rol, activo, pwd in USUARIOS:
        ...
        if u:
            u.nombre = nombre; u.username = username
            u.rol = rol; u.activo = activo
            u.password_hash = generate_password_hash(pwd)
    ...
    db.session.commit()
    rows = ''.join(
        '<tr><td>' + a + '</td><td>' + n + '</td><td>' + u + '</td><td>' + r + '</td><td>' + p + '</td></tr>'
        for a,n,u,r,p in log
    )
    return ('<html><head><meta charset="UTF-8"><style>'
            'body{font-family:Arial;background:#0f172a;color:#e2e8f0;padding:30px}'
            'table{border-collapse:collapse;width:100%;font-size:12px}'
            'td,th{padding:7px 12px;border-bottom:1px solid #2a3040;text-align:left}'
            'th{color:#4BB8C8;border-bottom:2px solid #4BB8C8}'
            'h2{color:#4BB8C8}a{color:#4BB8C8}'
            '</style></head><body>'
            '<h2>✅ Usuarios sincronizados</h2>'
            '<table><tr><th>Acción</th><th>Nombre</th><th>Username</th><th>Rol</th><th>Contraseña</th></tr>'
            + rows + '</table><br><a href="/admin/carga-v2">Cargar solicitudes</a>'
...
def carga_v2():
    ...
    if paso == 'menu':
        ...
        return (CSS + '<h2>Carga Masiva</h2>'
                ...
                '<p>Sincroniza los 20 usuarios del equipo.</p>'
                '<a class="btn" href="/admin/reset-passwords">Sincronizar usuarios</a></div>'
                '<div class="card"><h3>Paso 2 — Solicitudes históricas (lotes de 20)</h3>'
    ...
    if paso == 'solicitudes':
        ...
        inicio = lote * TAM
        fin    = min(inicio + TAM, len(all_rows))
        bloque = all_rows[inicio:fin]

        if not bloque:
            ts = Solicitud.query.count()
            return (CSS + '<h2>¡Carga completada!</h2>'
                    '<div class="card">Total solicitudes: <b>' + str(ts) + '</b></div>'
                    '<a class="btn g" href="/dashboard">Ir al Dashboard</a>')
        ...
        log = ['Lote ' + str(lote+1) + '/' + str(total_lotes) +
               ' (filas ' + str(inicio+2) + '-' + str(fin+1) + ')']
        ok = 0
        ...
        for r in bloque:
            ...
            sol = Solicitud(
                ...
                fecha_solicitud=f_sol or date(2026,1,1),
                fecha_captura=f_cap,
                ultima_actualizacion=f_cap,
            ...
            if estatus == 'Cerrada':
                cierre = dt2.combine(f_ent, dt2.min.time()) if f_ent else ahora
                sol.fecha_cierre = cierre; sol.usuario_cierre_id = admin_u.id
                sol.fecha_envio_cliente = cierre; sol.usuario_envio_id = admin_u.id
                sol.ultima_actualizacion = cierre

            db.session.add(sol)
            db.session.flush()
            db.session.add(Bitacora(solicitud_id=sol.id, usuario_id=admin_u.id,
                accion='Carga masiva historica. Status: ' + status_r))
            if resp:
                db.session.add(SolicitudIngeniero(
        ...
        btn = ('<a class="btn g" href="/admin/carga-v2?paso=solicitudes&lote=' + str(sig) + '">'
               'Siguiente lote (' + str(sig+1) + '/' + str(total_lotes) + ') →</a>' if fin < len(all_rows)
               else '<a class="btn g" href="/admin/carga-v2?paso=solicitudes&lote=' + str(sig) + '">Ver resumen</a>')

        salida  = CSS + '<h2>Lote ' + str(lote+1) + '/' + str(total_lotes) + '</h2>'
        salida += '<pre>' + '<br>'.join(log) + '</pre>' + btn
