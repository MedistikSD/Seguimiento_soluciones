...
class User(UserMixin, db.Model):
    ...
    rol = db.Column(db.String(30), nullable=False)  # hunter, soluciones, lider
...
class Solicitud(db.Model):
    ...
    def checkboxes_completos(self):
        return all([
            self.historial_surtido,
            self.inventario,
            self.maestro_productos,
            self.historial_recepcion,
...
class Documento(db.Model):
    ...
    nombre_guardado   = db.Column(db.String(255), nullable=False)   # filename on disk
    tipo_documento    = db.Column(db.String(20),  nullable=False)   # 'comercial' | 'soluciones'
    usuario_id        = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    ...
    activo            = db.Column(db.Boolean, default=True)         # False = versión antigua
